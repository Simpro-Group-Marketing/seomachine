"""Build plan fulfillment and machine reviews for the best-trade-jobs-california rewrite.
...
def fulfillment_excerpts(article: str) -> dict[str, str]:
    return {
        "ca-median-ranking-table": exact_slice(
            article,
            "| Rank | Trade | California median pay (top 10% earn) | Typical entry route | Credential or program to plan for |",
            "[CMS MODULE PLACEHOLDER | type: responsive trade pay table",
        ),
        "hands-on-trade-scope-rule": exact_slice(
            article,
            "Trades on this list are hands-on construction, installation, maintenance and repair occupations",
            "[IMAGE PLACEHOLDER | source: new original Simpro image, filename highest-paying-trades-in-california-salary-chart.webp",
        ),
        "five-part-trade-profiles": exact_slice(
            article,
            "### 1. Elevator and escalator installers and repairers",
            "### 2. Electrical power-line installers and repairers",
        ),
        "trade-choice-checklist": exact_slice(
            article,
            "1. How long a training route suits you",
            "Next, search California's [registered apprenticeship listings]",
        ),
    }
...
def article_review_findings() -> dict[str, list[dict[str, Any]]]:
    ...
    if "/blog/highest-paying-trade-jobs)" in article:
        result.setdefault("Internal Linker", []).append(
            finding(
                finding_id="article-internal-link-cannibalization",
                priority="high",
                category="internal_linking",
                location="article body",
                evidence_anchor="/blog/highest-paying-trade-jobs",
                recommendation="Remove the link to the national highest-paying trade jobs article, which the brief excludes to avoid cannibalization.",
                proof_risk="The brief prohibits linking the near-duplicate national article.",
            )
        )
...
def build_review(
    ...
    *,
    ...
    return build_machine_review(
        ...
        phase=phase,
        command="scripts/build_trade_jobs_ca_review_artifacts.py",

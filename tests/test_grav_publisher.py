...
def failing_readiness(gate_name="metric_proof_pack"):
    return {
        "passed": False,
        "gates": [
            {
...
class PublishPreflightTests(unittest.TestCase):
    ...
    def test_new_strategy_guards_block_before_grav_push(self):
        for gate_name in ("source_quality", "blog_strategy", "schema_handoff"):
            with self.subTest(gate_name=gate_name), tempfile.TemporaryDirectory() as tmp:
                ...
                sidecar = _write(tmp, "validation-post.md", "Validation sidecar\n")
                with patch(

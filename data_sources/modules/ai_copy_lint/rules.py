...
ERROR_RULES: list[tuple[str, Pattern[str], str, str]] = [
    ...
    ("editorial_process_leakage", re.compile(r"\b(?:the\s+brief\s+(?:asks|asked|calls|called|requires|required)|(?:right|wrong)\s+editorial\s+lane|this\s+(?:article|blog|post|draft)\s+uses|(?:this|the)\s+(?:article|blog|post|draft)\s+(?:does\s+not\s+name|doesn't\s+name|avoids\s+naming|routes?\s+readers|explains|covers|outlines|walks\s+through)|(?:a|the)\s+comparison\s+page\s+(?:evaluates|compares|covers|handles|owns)|(?:keep|keeps|keeping)\s+the\s+split\s+clean|(?:thin\s+strategy|noisy\s+copy)|(?:review|use|consult)\s+the\s+approved\s+[^.\n]{0,80}\b(?:outcome|proof|claim|metric|source)\b|(?:before|after)\s+you\s+move\s+into\s+platform\s+claims|(?:frontmatter|metadata|validation\s+sidecar|assembly\s+BOM|publish[- ]readiness)\s+(?:says|records|requires|allows|blocks))\b", re.IGNORECASE), "Editorial process notes are not allowed in public blog body copy.", "Move rationale to frontmatter, the validation sidecar, the BOM, or the editorial plan."),
...
COPY_AVOID_RULES: list[tuple[str, str, Pattern[str], str, str]] = [
    ...
    ("compressed_passive_participle", "error", re.compile(r"\b(?:software|platform|system|tool|agent|worker|workflow|process|role|feature|AI)\s+(?:assigned|defined|approved|connected|powered|enabled|driven|supported|managed|handled|built)\s+(?:[A-Za-z]+(?:[- ][A-Za-z]+){0,3}\s+)?(?:work|responsibilit(?:y|ies)|role|task|tasks|workflow|process|action|actions|capacity)\b", re.IGNORECASE), "Compressed passive phrasing hides the actor and weakens the sentence.", "Rewrite with an active verb, such as 'software carries recurring work' or 'teams assign the workflow to software'."),

    ...
    def require_selector_claim(
        ...
        *,
        ...
        for claim in self._claims:
            if claim.selector_id != selector and not claim.claim_id.endswith(selector):
                continue
...
def load_validated_claim_set(
    ...
    evidence_by_claim_id = {
        ...
        for item in _list_field(pack, "approved_claim_evidence")
...
def _validate_pack_and_receipt(pack: dict[str, Any], receipt: dict[str, Any]) -> None:
    ...
    _list_field(pack, "approved_claim_evidence")
...
def _approved_claim_from_records(
    ...
    selector_id = _same_required_string(evidence, decision, "selector_id")
    assertion = _same_required_string(evidence, decision, "assertion")
    use_mode = _same_use_mode(evidence, decision)
    source_hash = _same_required_string(evidence, decision, "source_hash")
    public_url = _same_public_url(evidence, decision)
    approval_source = _same_required_string(evidence, decision, "approval_source")
    if approval_source != APPROVAL_SOURCE:
        raise VaultClaimReceiptError("claim approval source must be connector_claim_result")
    ...
    if SIMPRO_SCOPE not in brand_scope:
        raise VaultClaimReceiptError("approved claim requires explicit Simpro scope")
    if str(decision.get("decision") or "").strip().casefold() != "approved":
...
def _same_public_url(evidence: dict[str, Any], decision: dict[str, Any]) -> str:
    evidence_value = str(evidence.get("public_url") or "").strip()
    decision_value = str(decision.get("public_url") or "").strip()
    if not evidence_value or not decision_value:
        raise VaultClaimReceiptError("approved claim requires a public URL")
    if evidence_value != decision_value:
        raise VaultClaimReceiptError("claim decision public URL does not match pack evidence")
    return evidence_value
...
def _receipt_revision(receipt: dict[str, Any]) -> str:
    declared = str(receipt.get("canonical_receipt_sha256") or "").strip()

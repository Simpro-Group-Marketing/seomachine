...
For AUS-oriented Simpro blog workflows, LLMs should consult `context/site-architecture-map.json` before choosing internal-link candidates or describing homepage, industry, solution, and feature clusters. This file is a supporting URL inventory only: use it for cluster awareness and candidate discovery, then validate the actual commercial pillar through `context/commercial-pillar-index.json` and final internal-link treatment through `context/internal-links-map.md`. It never replaces the commercial-pillar index and cannot prove keyword ownership, Semrush evidence, page performance, product claims, customer proof, or publish readiness.

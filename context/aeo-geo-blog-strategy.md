...
Hindsight Strategy Selection is sidecar-only. If Hindsight is not relevant, record `Status: not_applicable` or omit the block so BOM v4 records `not_applicable`. If Hindsight internal strategy informs the private plan, bind the internal-strategy pack/receipt/sidecar as `hindsight_strategy_evidence`, record `public_claim_use: prohibited` and `claim_support_allowed: false`, and do not cite it in public copy. Permitted use is private strategy shaping only: reader angle, section emphasis, contribution purpose, objections to answer, and commercial framing. The writer may execute the Hindsight-informed angle already frozen in the editorial plan, but cannot introduce new Hindsight-derived claims or a new unplanned angle inside public prose.
...
- **FAQ Source Policy**: Every visible FAQ URL needs one exact `FAQ Proof Map` row with `FAQ`, `URL`, `Source class`, `Competitor check`, and source-grounded `Support`.
  ...
  - Use regulators, standards bodies, universities, trade associations, independent research/editorial, non-competing expert sources, or owned product/resources pages when they directly support the answer.
  - Simpro-owned links can satisfy FAQ support when they directly support the answer and are classified as `owned_product`; they do not count toward the standard-blog non-owned external authority baseline.
...
- FAQ proof gate: `data_sources/modules/faq_proof_guard.py --fail-on error` must pass when FAQ answers are present. It requires a first-paragraph authoritative public link for fact-driven or high-risk answers, allows owned or non-owned support when mapped, prohibits competitor-owned FAQ sources, and validates the assigned lower-risk mode for other answers. A Source Map or FAQ Proof Map cannot replace a link required by `inline_required`.

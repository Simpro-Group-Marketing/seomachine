...
- **FAQ Source Policy**: Every visible non-owned FAQ URL needs one exact `FAQ Proof Map` row with `FAQ`, `URL`, `Source class`, `Competitor check`, source-grounded `Support`, `Classification artifact`, and `Classification hash`. The classification must be an unchanged repository-approved `simpro-source-classification/v1` artifact for the exact URL, allowed class, and independent publisher relationship. Sidecar labels alone are not authority.
...
- Competitive shortlist guard: `data_sources/modules/competitive_shortlist_guard.py --fail-on error` runs inside `/publish-readiness`. Competitor-aware articles must bind selected and rejected decisions, objective-specific reasons, connector resource IDs, and any public competitor claim IDs and URLs to the exact article plus validated context pack and receipt. SERP evidence may inform format but cannot authorize named competitors.
- Customer proof diversity guard: `data_sources/modules/customer_proof_diversity_guard.py --fail-on error` must pass before scoring or `/optimize`. Every `recent_uses_90d` value above 0 emits a warning. Reusing that source requires a source-specific `Reuse reason` plus an explicit selector-backed comparison against suitable approved zero-use proof for the same role. It also verifies non-case-study search routes, `Customer Proof Selection Decision`, and the current proof index and usage ledger.

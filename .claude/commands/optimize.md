...
All optimizer outputs and manual edits are content mutations. The closed sequence is `optimization` -> `post_optimization_scrub` -> `post_optimization_context_binding` -> `final_preflight_readiness`. The initial preflight remains bound to its immutable provisional BOM. Do not overwrite the BOM referenced by the prior preflight. Invoke `/publish-readiness` for the complete reseal after every mutation.
...
5. Finish the optimization receipt, rerun `/scrub` and the post-optimization Context Binding receipt stage, then invoke `/publish-readiness` for the complete reseal.

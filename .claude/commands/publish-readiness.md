...
After an optimization mutation, select the same PAA artifact, rewrite brief, or user-CSV-plus-AnswerSocrates-blocker build variant and add one `--optimizer-output "[agent-output]"` for every exact path already supplied through `--agent-output`, plus `--prior-preflight-readiness "[prior-preflight-readiness]"`. The optimizer output list must therefore contain the distinct output from every invoked registry agent in registry order; one aggregate report is invalid. Use the distinct post-optimization provisional and final BOM output paths described below, then run the same preflight -> finalize -> final steps. Do not combine research-input variants; the selected route determines the eligible input set.

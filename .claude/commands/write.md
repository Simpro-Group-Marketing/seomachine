...
After the article, research, proof, and receipts are complete, run the exact build -> preflight -> finalize -> final sequence:
...
The passed preflight automatically writes `research/preflight-readiness-[topic-slug]-[YYYY-MM-DD]-stage-receipt.json`. Final readiness writes a detached final-readiness attestation bound to the final BOM hash. It is not hashed back into the BOM. Both outputs declare `verification_scope: source_artifact`; they do not verify CMS rendering.

### After Optimization Mutations

All optimizer outputs and manual edits are content mutations. The closed sequence is `optimization` -> `post_optimization_scrub` -> `post_optimization_context_binding` -> `final_preflight_readiness`. Record the mutation, run the scrubber and Context Binding generator with their post-optimization stages, then rebuild the provisional BOM with every receipt, optimizer evidence, and `--prior-preflight-readiness "research/preflight-readiness-[topic-slug]-[YYYY-MM-DD].json"`. Write it to `research/blog-assembly-bom-[topic-slug]-[YYYY-MM-DD]-post-optimization.json`, run preflight, then finalize out of place to `research/blog-assembly-bom-[topic-slug]-[YYYY-MM-DD]-post-optimization-final.json` and run detached final readiness against that final BOM. Do not overwrite the BOM referenced by the prior preflight. Preflight-bound provisional BOMs are immutable.
